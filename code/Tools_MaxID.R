...
dF_GL <- function(r,parR,log=FALSE){
  ...
  ind <- !is.na(r)
  logval[!ind] <- NA
  logval[ind] <- log(alpha)+(r[ind]>=mu)*(log(beta1)+(beta1-1)*log(abs(r[ind]-mu))) + (r[ind]<mu)*(log(beta2)+(beta2-1)*log(abs(r[ind]-mu))) - alpha*( (r[ind]>=mu)*(abs(r[ind]-mu))^beta1-(r[ind]<mu)*(abs(r[ind]-mu))^beta2 )
  if(log){
    return( logval )	
  } else{
    return( exp(logval) )
...
upperF_GL <- function(x,parR,log=FALSE){
  ...
  logval[is.na(x)] <- NA
  ind <- !is.na(x)
  logval[ind] <- -alpha*( (x[ind]>=mu)*(abs(x[ind]-mu))^beta1-(x[ind]<mu)*(abs(x[ind]-mu))^beta2 )
  if(log){
    return( logval )	
  } else{
    return( exp(logval) )
...
mV12 <- function(x,h,parR,parGauss,reg=NULL,reg.t=NULL,log=FALSE,type="GS"){
  ...
  dGi <- function(xi){
    ...
    val <- integrate(fun,lower=low,upper=Inf,h=h,reg.t=reg.t,reg=reg,parR=parR,parGauss=parGauss,rel.tol=10^(-12),stop.on.error=FALSE)$value
    return(val)
  }
...
mVk <- function(x,k,h,parR,parGauss,reg=NULL,reg.t=NULL,log=FALSE,type="GS"){
  ...
  g <- function(xi,ki){
    ...
    if(type=="GS"){low=0}else{low=-Inf}
    val <- integrate(fun,lower=low,upper=Inf,h=h,reg=reg,reg.t=reg.t,parR=parR,parGauss=parGauss,rel.tol=10^(-12),stop.on.error=FALSE)$value
    return(val)
  }
  val <- c()
...
pG <- function(x,parR,log=FALSE,type="GS"){
  g <- function(xi){
    ...
    if(type=="GS"){low=0}else{low = -Inf}
    val<-integrate(fun,lower=low,upper=Inf,parR=parR,rel.tol=10^(-12),stop.on.error=FALSE)$value
    return(val)
  }
  logval <- c()
  I <- !is.na(x) 
...
dG <- function(x,parR,log=FALSE,type="GS"){
  ...
  val0[!I] <- NA
  logval <- log(val0)+pG(x,parR,log=TRUE,type = type)  #density is -V_1 exp(-V)
  if(log){
    return( logval )
  } else{
    return( exp(logval) )
...
fit.pw.parallel <- function(init,datU,coord,reg=NULL,reg.t=NULL,ptype=4,mtype="GS",cutoff=Inf,proppairs=1,fixed=rep(FALSE,3),optim=TRUE,gd=F,hessian=TRUE,sandwich=TRUE,eps=10^(-2),print.par.file=NULL,ncores=1,fit.save=FALSE,fit.load=FALSE,fit.file=NULL,maxit=300,...){ #fit max-id copula model by negative pairwise log likelihood
  ...
  if(optim==TRUE){
    ...
    pw.nllik2 <- function(par,pair2,index=nrow(datU)){
      ...
      par2.list <- get.par(par2,ptype,mtype=mtype)
...
fit.extremal.parallel <- function(init,datU,coord,reg=NULL,type=1,cutoff=Inf,proppairs=1,q=seq(0.05,0.95,0.1),fixed=rep(FALSE,5),gd=F,eps=10^(-2),ncores=1,...){ 
  ...
  #parGauss = list(lambda=NULL,lambda.t=NULL,a=NULL,theta=NULL,nu=NULL,type=type)
    ...
    pw.extremal2 <- function(par,ind){
      par2 <- init
      par2[which(!fixed)] <- par
      par2.list <- get.par(par2,type,mtype="GS")
